...
These names are reserved by the runner:
  ...
  - Variable names `BKP`, `BKX`, `ext`, `EXT`, `EXX`, `RCT`, `RCX`, `LDD`, `LDX`, `UDD`, `UDX`, `CAP`, `WRD`, `CHSET`, `OFFSET`, and `SANITIZE` are reserved in addition to the variables named above.
...
The command-line loops endlessly through steps 2-4 until interrupted or broken:
   ...
   4. process the key press if recognized or bypass it

The command-line stack counter `BKW` increments every time a new command-line is started by calling the `run_` function with no arguments.
